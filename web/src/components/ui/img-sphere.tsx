 ...
 * drag-to-rotate with momentum, auto-rotate, modal viewer, touch support).
 */
...
const SPHERE_MATH = {
  degreesToRadians: (d: number): number => d * (Math.PI / 180),
  radiansToDegrees: (r: number): number => r * (180 / Math.PI),
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes sphereScaleIn { from { transform: scale(0.8); opacity: 0 } to { transform: scale(1); opacity: 1 } }
      `}</style>
      <div
        ref={containerRef}
